...
def plot_timings(df, x, y, style,  title, xlabel, ylabel, output_file):
    ...
    sns.lineplot(data=df, x=x, y=y, style=style)
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.grid()
...
if __name__ == "__main__":
    ...
    for algo in {"p2p", "bcast", "p2p-i-la"}:
        plot_timings(df[df["algo"] == algo], "process_count", "duration", "m", f"Performance de {algo}", "Nombre de processus", "Temps [s]", f"{algo}.png")
